...
把服务分享到公网（如 cloudflared 隧道）时，建议设置访问口令：在 `config.local.json`（没有就新建）里加上：

```json
{ "access_code": "你的口令" }
```

重启服务后生效：访客打开网页会弹出口令输入框，输对一次 30 天内免输；口令输错会限速以防暴力尝试；所有数据与管理接口都在保护范围内。**不配置则完全不启用**——本机单人使用没有任何变化。口令仅存本机 `config.local.json`，不进 git。
